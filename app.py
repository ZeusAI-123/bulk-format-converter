...
---------------------------------------------------
Backend logic is fully embedded. No separate file needed.

Run:
    streamlit run app.py

Install:
    pip install streamlit Pillow pydub python-docx
...
DEFAULT_QUALITY = {
    "jpg": 85, "jpeg": 85, "webp": 80,
    "png": None,
    "mp3": 192, "ogg": 192, "aac": 192,
    "wav": None, "flac": None,
}
...
def convert_doc(input_path: Path, output_path: Path):
    ...
    if input_ext == ".docx" and output_ext == ".txt":
        ...
        raise ValueError(f"Unsupported doc conversion: {input_ext} to {output_ext}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, uf in enumerate(uploaded_files):
            ...
            if file_type is None:
                results.append(ConversionResult(uf.name, "", "skipped", "Unrecognised format"))
                continue
            ...
            if target_format not in get_valid_outputs(file_type):
                results.append(ConversionResult(uf.name, "", "skipped", f"Cannot convert {file_type} to .{target_format}"))
                continue
            ...
            try:
                if file_type == "image":   convert_image(in_path, out_path, quality)
                elif file_type == "audio": convert_audio(in_path, out_path, quality)
                elif file_type == "doc":   convert_doc(in_path, out_path)
